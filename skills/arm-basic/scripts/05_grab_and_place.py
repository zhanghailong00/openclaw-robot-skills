...
功能：一次调用完成：开爪 → 转向 → 悬停 → 下降 → 夹取 → 抬起 → [安全位] → 放置位 → 下降 → 松开 → 抬起 → [安全位]
硬件：4-DOF 机械臂（UART 串口，/dev/ttyUSB1）
优势：相比单独调用多个脚本，省掉 4-5 次 Python 启动时间
安全：抓取和放置后都经过安全位过渡，避免路径横甩
...
用法：
  ...
  # 不经过安全位（直接路径，适合连续操作）
  python3 05_grab_and_place.py --grab_x 154 --grab_y -50 --grab_z -31 --place_x 212 --place_y 62 --place_z 62 --no-safe
...
parser.add_argument("--no-safe", action="store_true", help="不经过安全位（直接路径）")
...
try:
    ...
    # 7. 经过安全位过渡（避免路径横甩）
    if not args.no_safe:
        arm.move(SAFE_POS, t=args.t, wait=True)
    ...
    if args.place_x is not None and args.place_y is not None and args.place_z is not None:
        ...
        result = {
            ...
            "safe_pos": SAFE_POS if not args.no_safe else None
        }
    else:
        # 没有指定放置位置，移到安全位
        arm.move(SAFE_POS, t=args.t, wait=True)
